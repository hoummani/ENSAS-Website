<template>
  <div class="pa-4">
    <v-layout class="pa-3" wrap row>
      ...
      ></v-select>
      <v-spacer></v-spacer>
      <v-btn color="secondary">Ajouter Un Group</v-btn>
    </v-layout>
    <v-card class="pa-2 mb-2 pl-5" v-for="item in jurys" :key="item.number" v-ripple="{ class: `indigo--text lighten-2` }">
      
      <v-layout row wrap class="pa-3">
        <v-flex xs12 sm8 md9>
          <div class="caption grey--text">Numero de group</div>
          <div class="body-2 font-weight-bold">Groupe N : {{item.number}}</div>
        </v-flex>
